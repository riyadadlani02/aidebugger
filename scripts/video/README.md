# Demo video pipeline

`../../docs/aidebugger-demo.mp4` is rendered from these, not edited by hand. Regenerate it when the
verified counts or framework versions in `scene.html` go stale.

`scene.html` is a deterministic player: `window.seek(t)` renders the exact state at time `t`, so
every frame is reproducible rather than dependent on animation timing. `frames.json` is a real
capture from `examples/langchain_agent.py` run under aidebugger — the console scene shows genuine
frames, not mock-ups.

```bash
npm i puppeteer-core
python3 -m http.server 8912 &          # scene.html fetches frames.json, so file:// won't do
node render.js                          # 2640 PNGs at 2560x1440
ffmpeg -framerate 30 -i frames/%05d.png -vf scale=1920:1080:flags=lanczos \
       -c:v libx264 -preset slow -crf 20 -pix_fmt yuv420p -movflags +faststart \
       ../../docs/aidebugger-demo.mp4
```

`render.js` points at `/Applications/Google Chrome.app`; change `CHROME` elsewhere.
