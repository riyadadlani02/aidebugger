"""Build docs/live/ — the console driven by a real aidebugger running in the visitor's browser.
...
<script>
// The console's transport, pointed at an aidebugger engine in this tab instead of one over HTTP.
// Same endpoints, same shapes — the page cannot tell the difference, which is the point.
const AIDEBUGGER_LIVE = (async () => {
  const py = await loadPyodide();
  py.FS.mkdirTree('/lib/aidebugger/trapsets');
  const put = async (path, url) => py.FS.writeFile(path, await (await fetch(url)).text());
  await Promise.all([
    ...PKG_FILES.map(f => put('/lib/aidebugger/' + f, 'pkg/aidebugger/' + f)),
    ...PKG_TRAPSETS.map(f => put('/lib/aidebugger/trapsets/' + f, 'pkg/aidebugger/trapsets/' + f)),
...
window.fetch = (input, init) => {
  ...
  return AIDEBUGGER_LIVE.then(api => {
...
(async () => {
  ...
  const api = await AIDEBUGGER_LIVE;
  $$('boot').hidden = true;
  $$('ready').hidden = false;
...
import backend
from aidebugger import render
from aidebugger.engine import Engine
...
def main():
    ...
    (PKG / "aidebugger" / "trapsets").mkdir(parents=True, exist_ok=True)
    for name in PY_FILES:
        shutil.copy(ROOT / "aidebugger" / name, PKG / "aidebugger" / name)
    for name in TRAPSETS:
        shutil.copy(ROOT / "aidebugger" / "trapsets" / name, PKG / "aidebugger" / "trapsets" / name)
    ...
    html = html.replace("<title>aidebugger console</title>",
                        "<title>aidebugger console — live in your browser</title>")
