"""Dropped on PYTHONPATH by `aidebugger run` - installs traps with zero target code change."""
import os
import sys
...
try:
    ...
    print(f"[aidebugger] chained sitecustomize failed: {exc}", file=sys.stderr)
...
try:
    ...
    print(f"[aidebugger] failed to install: {exc}", file=sys.stderr)
