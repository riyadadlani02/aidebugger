"""aidebugger CLI - a dumb HTTP client plus the `run` launcher. No logic lives here."""
import argparse
import json
import os
import pathlib
...
PORTFILE = pathlib.Path(tempfile.gettempdir()) / "aidebugger.port"


def _port(explicit=None):
    if explicit:
        return explicit
    if os.environ.get("AIDEBUGGER_PORT"):
        return int(os.environ["AIDEBUGGER_PORT"])
    if PORTFILE.exists():
        return int(PORTFILE.read_text().strip())
    sys.exit("no target port: pass --port, or launch with `aidebugger run`")
...
def cmd_run(args):
    ...
    if not command:
        sys.exit("usage: aidebugger run -- <command...>")
    pkg = pathlib.Path(__file__).resolve().parent
    env = dict(os.environ)
    # pkg.parent keeps aidebugger importable when running from a checkout, not just an install.
    env["PYTHONPATH"] = os.pathsep.join(
        [str(pkg / "_shim"), str(pkg.parent), *filter(None, [env.get("PYTHONPATH")])]
    )
    env["AIDEBUGGER_PORT"] = str(args.port)
    env["AIDEBUGGER_PORTFILE"] = str(PORTFILE)
...
def main(argv=None):
    p = argparse.ArgumentParser(prog="aidebugger", description=__doc__)
    p.add_argument("--port", type=int, help="target control port")
    sub = p.add_subparsers(dest="cmd", required=True)

    run = sub.add_parser("run", help="launch a process with aidebugger installed")
